...
/// <summary>
/// Strips EXIF, XMP and ICC color-profile boxes from HEIF / HEIC files at the
/// ISO base media file format (ISOBMFF / ISO 14496-12) box level, preserving the
/// image bitstream (<c>mdat</c>) and the structural metadata boxes (<c>hdlr</c>,
/// <c>pitm</c>, <c>iloc</c>, <c>iinf</c>, <c>iprp</c>, …) bit-for-bit.
/// </summary>
/// <remarks>
/// <para>
/// HEIF is an ISOBMFF container. Structure:
/// <code>
...
/// <c>8532C9A2-3B9A-11E4-B6A2-0401E0CBBFCE</c> (the format most iOS devices
/// emit). XMP is carried in a <c>uuid</c> box with UUID
/// <c>BE7ACFCB-97A9-42E8-9C71-999491E3AFAC</c> or, less commonly, in a
/// <c>mime</c> box whose content type is <c>application/rdf+xml</c>. ICC color
/// profiles live in a <c>colr</c> box with a <c>rICC</c> or <c>prof</c>
...
/// structural box (<c>hdlr</c>, <c>iloc</c>, <c>iinf</c>, <c>iprp</c>,
...
/// </para>
/// </remarks>
public sealed class HeifMetadataCleaner : IFileMetadataCleaner
{
    private static readonly string[] Extensions = [".heic", ".heif"];

    /// <summary>HEIF/HEIC brands accepted in the <c>ftyp</c> compatible-brands list.</summary>
    private static readonly HashSet<string> HeifBrands = new(StringComparer.Ordinal)
    {
        "heic", "heix", "heim", "heis", "hevc", "hevx", "mif1", "msf1", "jpeg",
    };

    /// <summary>UUID Apple emits for EXIF data inside HEIF/HEIC <c>meta</c>.</summary>
    ...
    /// <summary>UUID used for XMP data inside HEIF/HEIC <c>meta</c>.</summary>
    ...
    private static byte[] ReadFile(string path)
    {
        ...
        if (!IsValidHeif(bytes))
        {
            throw new MetadataStripException($"Not a valid HEIF/HEIC file: {path}") { FilePath = path };
        }
    ...
    /// that lists at least one HEIF/HEIC brand (major_brand or any
    /// compatible_brand).
    /// </summary>
    private static bool IsValidHeif(byte[] bytes)
    {
        if (bytes.Length < 16)
        {
            return false;
        ...
        if (HeifBrands.Contains(majorBrand))
        {
            return true;
        }
    ...
    private static byte[] Process(byte[] data, MetadataCleanOptions options, List<MetadataEntry> removed, bool inspectOnly)
    {
        ...
        try
        {
            while (pos < data.Length)
            {
                ...
                else if (IsStrippableType(type))
                {
                    // inspectOnly: only record removal candidates, no copy.
                    // (Top-level metadata outside `meta` is not produced by HEIF encoders,
                    //  so this branch is effectively never taken; we keep the structure for
                    //  future-proofing if a non-conformant encoder surfaces EXIF at the
                    //  top level.)
                }
        ...
        {
            throw new MetadataStripException("Corrupt HEIF/HEIC structure encountered while stripping metadata.", ex);
        }
    ...
    /// <c>iinf</c>, <c>iprp</c>, …) and only true for the four well-known
    ...
    private static BoxHeader ReadBoxHeader(byte[] data, int pos)
    {
        ...
        {
            throw new MetadataStripException("Truncated HEIF/HEIC box header.");
        }
        ...
        if (size32 == 1)
        {
            ...
            {
                throw new MetadataStripException("Truncated HEIF/HEIC largesize header.");
            }
        ...
        else if (size32 == 0)
        {
            ...
            throw new MetadataStripException("HEIF/HEIC box with size==0 is not supported.");
        }
        else
        {
            boxSize = size32;
        ...
        {
            throw new MetadataStripException("HEIF/HEIC box size is smaller than its header.");
        }

        if (pos + boxSize > data.Length)
        {
            throw new MetadataStripException("Truncated HEIF/HEIC box payload.");
        }
    ...
    /// <summary>Top-level strippable fourccs (used to short-circuit inspect-only at the top level).</summary>
    private static bool IsStrippableType(string type) => type is "Exif" or "uuid" or "colr" or "mime";
