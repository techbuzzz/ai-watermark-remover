...
description: >
  ...
  a JPEG / PNG / PDF / DOCX / HTML / WebP / TIFF file before sharing, or
  wants to know whether a text blob carries a Claude / Gemini / OpenAI /
  DeepSeek / Grok watermark. Routes to the `watermarkremover` MCP server
  (eight tools: clean_text, clean_markdown, clean_file, clean_image,
  detect_text, detect_markdown, inspect_file, detect_watermark) or — when
  MCP is not registered — to the matching `watermarkremover` CLI
  subcommand. The project also ships three MiniMax Code slash commands
  under `commands/` (`/wr-clean-text`, `/wr-clean-file`, `/wr-detect`)
  for one-shot invocation.
license: MIT
compatibility: minimax-code
---

# Skill — `watermark-remover`

Master skill for the WatermarkRemover integration. This is the entry
point: it teaches the MiniMax Code agent **when** to think about
WatermarkRemover and points at the right MCP tool / slash command / CLI
invocation. The five per-format skills
(`watermark-clean-text`, `watermark-clean-markdown`,
`watermark-clean-file`, `watermark-clean-image`, `watermark-detect`)
are co-located in the same plugin under `skills/` and have the deep
...
| Inpaint a visible logo / watermark             | `clean_image` MCP tool (no slash command — needs an image)     |
...
This plugin ships three slash-command files under `commands/`:
...
> **Status.** The V1 plugin manifest format does not yet declare
> `commands` as a first-class capability, so the slash commands are
> shipped as forward-looking files. If a future MiniMax Code version
> auto-discovers `commands/*.md` (the same convention OpenCode uses),
> they will work as-is. Until then, the agent invokes the underlying
> MCP tools directly through the routing table above.

## MCP server

When the `watermarkremover` plugin is enabled, the MCP server is
auto-registered with the agent as a stdio child process. The agent
gets the full set of 8 tools: `clean_text`, `clean_markdown`,
`clean_file`, `clean_image`, `detect_text`, `detect_markdown`,
`inspect_file`, `detect_watermark`. Prefer the MCP tools over the CLI
when they are available — they're one round-trip and they don't spawn
a subprocess.

If the `watermarkremover` binary is missing on `$PATH`, the MCP server
entry in `servers.mcp.json` will fail to start. The plugin's discovery
screen will flag the error; the fix is to install the binary
([`README → Installation`](../../../README.md#-installation)) and
re-enable the plugin.

## CLI fallback

When the MCP server is not available (or the agent explicitly wants to
shell out), the canonical command shape is:
...
Agent: invokes `/wr-clean-text "Hello world"` (or the `clean_text`
MCP tool with `stripInvisible: true`), gets back `"Helloworld"` plus
a `removed` list naming `ZWSP x2`.
...
- **Binary not on PATH** — the plugin's MCP entry will fail at startup
  with a "command not found" error. Tell the user to install the
  `watermarkremover` binary (see
  [README → Installation](../../../README.md#-installation)) and
  re-enable the plugin.
...
- Per-skill deep-dive: [`docs/SKILLS.md`](../../../docs/SKILLS.md) and
  the five folders under [`skills/`](../../../skills/).
- MCP server reference: [`docs/MCP.md`](../../../docs/MCP.md).
- MiniMax Code install reference: [`docs/MINIMAX-CODE.md`](../../../docs/MINIMAX-CODE.md).
