...
- **PPTX + XLSX metadata cleaners
  ...
  JPEG / PNG / WebP / TIFF / PDF / DOCX / HTML / EPUB / HEIF / AVIF.
  Both cleaners are built on the same Open XML SDK 3.2.0 already pinned
  in `Directory.Packages.props`:

    - A new internal `OpenXmlCoreMetadataCleaner` helper handles the
      parts shared by all three Microsoft Open XML formats:
      `core.xml` (the `PackageProperties` / `dc:creator` /
      `cp:lastModifiedBy` set), `app.xml` (`ExtendedFilePropertiesPart`),
      and `custom.xml` (`CustomFilePropertiesPart`). The helper finds
      the parts through the package's `Parts` collection and
      `DeletePart` on each, so it works against the common
      `OpenXmlPackage` base class without needing per-document-type
      accessors.
    - `PptxMetadataCleaner` then walks every per-slide
      `PowerPointCommentPart` (and the presentation-wide
      `authorsPart` of type `PowerPointAuthorsPart`) and deletes them,
      so slide comments and their authorship go away with the rest of
      the metadata. The slide content (text, shapes, layout) is
      preserved byte-for-byte.
    - `XlsxMetadataCleaner` then walks every per-worksheet
      `WorksheetCommentsPart` + `WorksheetThreadedCommentsPart` and
      the workbook-wide `CommentAuthorsPart` and deletes them all,
      so cell comments and their authorship go away too. Cell values,
      formulas, and the shared string table are preserved.

  Both new cleaners re-validate the output as a real
  `PresentationDocument` / `SpreadsheetDocument` and surface
  `MetadataStripException` for corrupt / non-Office-Open-XML inputs.
  The router is updated: `AddWatermarkRemoverMetadata` registers both
  new cleaners; `FileCleanerRouter` resolves `.pptx` and `.xlsx`
  (case-insensitive) to them. The package description and tags on
  `WatermarkRemover.Metadata.csproj` now list PPTX + XLSX. README
  adds the two formats to the supported-format list, the metadata
  section, and the project-tree one-liner. **20 new xUnit tests** in
  `WatermarkRemover.Metadata.Tests` (7 PPTX cleaner tests + 7 XLSX
  cleaner tests covering Inspect / Clean / round-trip / content
  preservation / corrupt input / missing file / `CanHandle`; plus 6
  router theory + fact rows for `.pptx`/`.PPTX`/`.xlsx`/`.XLSX`).
  Build clean (0 warnings, 0 errors), **592 xUnit tests total**
  (81 + 35 + 9 + 121 + 39 + 307), all green.
...
- **NuGet packaging for the four library projects (WR-P010)** — the
  ...
  repo. The four library projects (`WatermarkRemover.Core`,
  `.Text`, `.Metadata`, `.Image`) are now `IsPackable=true` with
  full NuGet metadata: `PackageId`, `PackageVersion` (1.0.0,
  in lockstep with the CLI), `PackageReadmeFile` (a per-project
  `README.md` describing the public surface and a usage example),
  `PackageIcon` (a new 128×128 PNG at
  `src/tools/watermarkremover-nuget-icon.png`), `PackageLicenseExpression`
  = `MIT`, and `PackageProjectUrl` pointing at the GitHub repo.
  The packages are strong-named via a new
  `src/tools/watermarkremover.snk` (2048-bit RSA, generated by the
  new `scripts/SnkGen` helper that wraps
  `RSACryptoServiceProvider.ExportCspBlob(true)` — the only .NET
  API that natively emits the CSP `PRIVATEKEYBLOB` shape the .NET
  compiler's `Csc` task expects from
  `<AssemblyOriginatorKeyFile>`); `<SignAssembly>true</SignAssembly>`
  on every library plus `<PublicSign>true</PublicSign>` for
  cross-platform builds (so the signing step works on Linux/macOS
  CI runners without the Windows-only private key container
  service). A new `scripts/SnkGen` project documents and ships
  the generator so any contributor can rotate the key without
  needing `sn.exe` (the Windows SDK tool, which is the only other
  source of valid .snk files and is not available in this
  environment). 39 new xUnit tests in
  `WatermarkRemover.CLI.Tests/NuGetPackagingTests` guard the
  contract: the strong-name key exists, is non-empty, and has a
  valid PUBLICKEYBLOB header (`bType=0x06`, `bVersion=0x02`,
  `"RSA1"` magic, sane bit length); the shared icon is a real PNG
  of exactly 128×128; each of the 4 csproj files declares
  `IsPackable=true`, `PackageId` matching the assembly name,
  non-empty `PackageVersion`, `PackageReadmeFile=README.md`,
  `PackageIcon=watermarkremover-nuget-icon.png`,
  `PackageLicenseExpression=MIT`, an HTTPS `PackageProjectUrl`,
  `SignAssembly=true` against `..\tools\watermarkremover.snk`,
  AND a `<None Pack="true">` item for both the README and the icon
  (without the latter, `dotnet pack` errors with `NU5039` /
  `NU5046`). Build clean (0 warnings, 0 errors), **477 xUnit
  tests** (81 + 35 + 26 + 9 + 39 + 287) + 0 Node tests, all green.
  `dotnet pack` for each of the 4 projects produces a valid
  `.nupkg` with the expected id, version, README, icon, and
  `<dependency>` graph.
